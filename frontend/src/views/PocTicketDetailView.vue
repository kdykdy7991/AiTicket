...
function buttonType(a:TicketAction){return ['reject','return','cancel'].includes(a)?'danger':'primary'} function buttonLabel(a:TicketAction){return a==='return'&&ticket.value?.state==='pending_routing'?'驳回':actionLabel(a)} function formatTime(v:string|null){return v?dayjs(v).format('YYYY-MM-DD HH:mm'):'—'} function stateIndex(s:PocState){return MAIN_FLOW_STATES.indexOf(s)}
const submitLabel=computed(()=>currentAction.value==='route'?'确认并流转':`确认${buttonLabel(currentAction.value)}`); const commentRequired=computed(()=>['reject','return','cancel'].includes(currentAction.value)); const showComment=computed(()=>!['submit_plan','submit_analysis','pass_review','register_defect','resubmit'].includes(currentAction.value)); const commentLabel=computed(()=>currentAction.value==='route'?'确认说明（可选）':commentRequired.value?'原因':'意见');
...
function openAction(a:TicketAction){currentAction.value=a;comment.value='';Object.keys(actionPayload).forEach(k=>delete actionPayload[k]);if(a==='return'&&ticket.value){actionPayload.return_to_state=({pending_routing:'pending_approval',pending_plan_confirmation:'planning',pending_quality_review:'processing'} as Partial<Record<PocState, PocState>>)[ticket.value.state]}dialogVisible.value=true}
