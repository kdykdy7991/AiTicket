      ...
      <div class="detail-grid">
        <main>
          <el-card shadow="never" class="section"><template #header><strong>问题信息</strong></template>
            <div class="fields"><Field label="提出人" :value="ticket.proposer"/><Field label="提出部门" :value="ticket.proposer_department"/><Field label="客户名称" :value="ticket.customer_name"/><Field label="产品线" :value="ticket.product_line"/><Field label="问题类型" :value="ticket.problem_type"/><Field label="发生时间" :value="formatTime(ticket.occurred_at)"/><Field label="发生地点" :value="ticket.location"/><Field label="坐标" :value="coordinates"/><Field label="批准人" :value="ticket.approver_name"/><Field label="闭环要求" :value="ticket.closure_requirement" wide/><Field label="设备信息" :value="ticket.device_info" wide/><Field label="问题现象" :value="ticket.description" wide/></div>
          </el-card>
          ...
          <el-card v-if="hasReview" shadow="never" class="section"><template #header><strong>质量评审</strong></template><div class="fields"><Field label="验证状态" :value="verificationLabel"/><Field label="验证结论" :value="ticket.verification_conclusion" wide/><Field label="质量评审结果" :value="ticket.quality_review_result" wide/></div></el-card>
        ...
        <aside>
          <el-card shadow="never" class="section sticky"><template #header><strong>当前处理</strong></template>
            <dl><dt>责任角色</dt><dd>{{ roleLabel(ticket.current_responsible_role) }}</dd><dt>所属分系统</dt><dd>{{ ticket.skill_group_name || '—' }}</dd><template v-if="ticket.subsystem_owner_name"><dt>分系统负责人</dt><dd>{{ ticket.subsystem_owner_name }}</dd></template></dl>
            <el-alert v-if="ticket.state === 'returned'" type="error" :closable="false" title="问题已退回，请根据流程记录修订后重新提交" />
            <div v-if="ticket.allowed_actions.length" class="action-list"><el-button v-for="action in ticket.allowed_actions" :key="action" :type="buttonType(action)" @click="openAction(action)">{{ buttonLabel(action) }}</el-button></div>
...
function buttonType(a:TicketAction){return ['reject','return','cancel'].includes(a)?'danger':'primary'} function buttonLabel(a:TicketAction){return a==='return'&&['pending_routing','pending_final_approval'].includes(ticket.value?.state||'')?'驳回':actionLabel(a)} function formatTime(v:string|null){return v?dayjs(v).format('YYYY-MM-DD HH:mm'):'—'} function stateIndex(s:PocState){return MAIN_FLOW_STATES.indexOf(s)}
const submitLabel=computed(()=>currentAction.value==='route'?'确认并流转':`确认${buttonLabel(currentAction.value)}`); const commentRequired=computed(()=>['reject','return','cancel'].includes(currentAction.value)); const showComment=computed(()=>!['submit_plan','submit_analysis','pass_review','resubmit'].includes(currentAction.value)); const commentLabel=computed(()=>currentAction.value==='route'?'确认说明（可选）':commentRequired.value?'原因':'意见');
const canSubmit=computed(()=>{const p=actionPayload,a=currentAction.value;if(commentRequired.value&&!comment.value.trim())return false;if(a==='route')return !!p.skill_group_id&&!!p.subsystem_owner_id;if(a==='submit_plan')return !!p.long_term_measure?.trim()&&!!p.planned_completion_at;if(a==='submit_analysis')return !!p.initial_investigation?.trim()&&!!p.root_cause?.trim()&&!!p.analysis_report?.trim();if(a==='pass_review')return !!p.verification_status&&!!p.verification_conclusion?.trim()&&!!p.quality_review_result?.trim();return true})
...
function openAction(a:TicketAction){currentAction.value=a;comment.value='';Object.keys(actionPayload).forEach(k=>delete actionPayload[k]);if(a==='return'&&ticket.value){actionPayload.return_to_state=({pending_routing:'pending_approval',pending_plan_confirmation:'planning',pending_quality_review:'processing',pending_final_approval:'pending_quality_review'} as Partial<Record<PocState, PocState>>)[ticket.value.state]}dialogVisible.value=true}
...
.section-header{display:flex;align-items:center;justify-content:space-between}.upload-button{padding:7px 12px;border:1px solid var(--color-primary);border-radius:7px;color:var(--color-primary);font-size:13px;cursor:pointer}.upload-button input{display:none}
</style>
