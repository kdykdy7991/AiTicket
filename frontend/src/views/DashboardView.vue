  ...
  <div v-loading="loading" class="overview-page">
    ...
    <section class="metrics-grid">
      ...
      <article class="metric-card panel rate-card">
        <div class="metric-title"><i class="green"><Refresh/></i><span>闭环率</span><el-tooltip placement="top" effect="dark" content="已解决数 ÷ 有效问题总数；总数为 0 时显示 —"><InfoFilled class="tip-icon"/></el-tooltip></div>
      ...
      <article class="metric-card panel rate-card overdue">
        <div class="metric-title"><i><AlarmClock/></i><span>未按时闭环率</span><el-tooltip placement="top" effect="dark" content="超时问题数 ÷ 有计划完成时间的问题总数。超时：计划完成时间已过且流程仍未走到质量评审；分母为 0 时显示 —"><InfoFilled class="tip-icon"/></el-tooltip></div>
...
import { AlarmClock, CircleCheck, FirstAidKit, FolderOpened, Histogram, InfoFilled, Management, Refresh, Tickets, Tools, UserFilled, VideoPause } from '@element-plus/icons-vue'
...
const statusCards=computed(()=>[
 {label:'总数量',value:stats.value?.effective_total||0,note:'有效问题总数（不含已撤销）',tip:'POC 流程中非草稿、未撤销的有效问题总数',icon:Tickets,color:'#46505e',tint:'#eef0f3'},
 {label:'已解决',value:stats.value?.resolved_count||0,note:'质量审核结论为已解决',tip:'已通过质量评审，验证结论为「已解决」',icon:CircleCheck,color:'#4f7c62',tint:'#eaf3ed'},
 {label:'临时解决',value:stats.value?.temporarily_resolved_count||0,note:'已采取临时处置措施，尚未走到质量评审',tip:'质量评审前的阶段，已填写临时处置措施',icon:FirstAidKit,color:'#a66f35',tint:'#f8efe4'},
 {label:'挂起',value:stats.value?.suspended_count||0,note:'质量评审结论为挂起',tip:'已通过质量评审，验证结论为「挂起」',icon:VideoPause,color:'#7a6f9e',tint:'#efedf6'},
